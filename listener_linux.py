...
def get_system_info(client_socket):
    ...
    commands = [
        ("whoami", "user"),
        ("systeminfo | findstr /B /C:\"OS Name\" /C:\"OS Version\"", "os"),
    ]
    ...
    for cmd, key in commands:
        ...
        if response:
            if key == "user":
                ...
                cleaned = lines[-1] if lines else "Unknown" 
            else:
        ...
        for interface in netifaces.interfaces():
            ...
            if netifaces.AF_INET in addrs:
                for addr in addrs[netifaces.AF_INET]:
                    available_ips.append(addr['addr'])
...
def main():
    ...
    try:
        ...
        try:
            ...
            if e.errno == 13:
                log_msg(f"Error: Port {LS_PORT} requires root privileges", "error")
            elif e.errno == 98:
